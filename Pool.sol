...
contract ExtinctionPool {
    ...
    struct UserInfo 
    {
        uint256 amount;                     // How many TestToken tokens the user has provided.
    ...
    IERC20 public immutable TEST_TOKEN;              
    ...
    mapping (address => UserInfo) public userInfo;     // Info of each user that stakes TestToken tokens.

    event Withdraw(address indexed user, uint256 amount);
    event Deposit(address indexed user, uint256 amount);
    event Burn(uint256 amount);

    constructor(IERC20 _TEST_TOKEN, IERC20 _REWARD, uint256 _rewardPerBlock, uint256 _startBlock, uint256 _endBlock, uint256 _lockBlock) public {
        require(address(_TEST_TOKEN) != address(0), "_TEST_TOKEN address not set!");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 testSupply = TEST_TOKEN.balanceOf(address(this));
        if (block.number > poolInfo.lastRewardBlock && testSupply != 0) {
            uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
            uint256 tokenReward = multiplier.mul(rewardPerBlock);
            accRewardPerShare = accRewardPerShare.add(tokenReward.mul(1e12).div(testSupply));
        }
    ...
    function updatePool() public {
        ...
        uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock);
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(testSupply));
        poolInfo.lastRewardBlock = block.number;
    }

    /**
     * @dev Deposit TEST_TOKEN tokens to the Extinction Pool for rewards allocation and/or withdraw outstanding rewards.
     * @param _amount Amount of TEST_TOKEN tokens to deposit.
     */
    function transact(uint256 _amount) public {
        ...
        if (block.number < lockBlock && _amount != 0) {
            TEST_TOKEN.safeTransferFrom(address(msg.sender), address(this), _amount);
            user.amount = user.amount.add(_amount);
            user.rewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(1e12);
            emit Deposit(msg.sender, _amount);
        }
        if (block.number >= endBlock) {
            TEST_TOKEN.safeTransfer(burnAddress, user.amount);
            user.amount = 0;
            user.rewardDebt = user.amount.mul(poolInfo.accRewardPerShare).div(1e12);
