...
contract FossilFarms is Ownable {
    ...
    struct PoolInfo 
    {
        ...
        uint256 lastRewardBlock;        // Last block number that TEST_TOKEN distribution occured.
        uint256 accTestPerShare;        // Accumulated TEST_TOKEN per share, times 1e12. See below.
    }

    IERC20 public TEST_TOKEN;                 // TEST_TOKEN token
    PoolInfo[] public poolInfo;         // Info of each pool.
    uint256 public testPerBlock;        // TEST_TOKEN tokens created per block.
    uint256 public startBlock;          // The block number at which TEST_TOKEN distribution starts.
    uint256 public endBlock;            // The block number at which TEST_TOKEN distribution ends.
    ...
    constructor(IERC20 _TEST_TOKEN, uint256 _testPerBlock, uint256 _startBlock, uint256 _endBlock) public {
     ...
     * @param _withUpdate Whether to update all LP token contracts. Should be true if TEST_TOKEN distribution has already begun.
     */
    ...
    /**
     * @dev Update the given pool's TEST_TOKEN allocation point. Can only be called by the owner.
    ...
    /**
     * @dev View function to see pending TEST_TOKEN on frontend.
     * @param _pid ID of a specific LP token pool. See index of PoolInfo[].
     * @param _user Address of a specific user.
     * @return Pending TEST_TOKEN.
     */
    ...
    /**
     * @dev Deposit LP tokens to the Fossil Farm for TEST_TOKEN allocation.
     * @param _pid ID of a specific LP token pool. See index of PoolInfo[].
    ...
    /**
     * @dev Safe transfer function, just in case if rounding error causes the Fossil Farm to not have enough TEST_TOKEN.
     * @param _to Target address.
     * @param _amount Amount of TEST_TOKEN to transfer.
     */
    function safeTestTransfer(address _to, uint256 _amount) internal {
        uint256 testBalance = TEST_TOKEN.balanceOf(address(this));
        if (_amount > testBalance) {
            TEST_TOKEN.transfer(_to, testBalance);
        } else {
            TEST_TOKEN.transfer(_to, _amount);
        }
    }
    ...
    /**
     * @dev Views total number of TEST_TOKEN tokens deposited for rewards.
     * @return TEST_TOKEN token balance of the Fossil Farm.
     */
    function balance() public view returns (uint256) {
        return TEST_TOKEN.balanceOf(address(this));
    }

    /**
     * @dev Transfer TEST_TOKEN tokens.
     * @return Success.
     */
    function transfer(address to, uint256 value) external onlyOwner returns (bool) {
        return TEST_TOKEN.transfer(to, value);
    }

    /**
     * @dev Define last block on which TEST_TOKEN reward distribution occurs.
    ...
    /**
     * @dev Update TEST_TOKEN per block.
     * @return TEST_TOKEN per block.
     */
    function setTestPerBlock(uint256 _testPerBlock) external onlyOwner returns (uint256) {
        require(_testPerBlock > 0, "Test per Block must be greater than 0.");
        massUpdatePools();
        testPerBlock = _testPerBlock;
        return testPerBlock;
    }
